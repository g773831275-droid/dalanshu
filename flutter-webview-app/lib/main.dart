...
class _WebShellState extends State<WebShell> {
  ...
  Future<List<String>> _selectFiles(FileSelectorParams params) async {
    try {
      ...
      final mediaType = acceptedMediaType(params.acceptTypes);
      final source = params.isCaptureEnabled
          ? ImageSource.camera
          : ImageSource.gallery;
      final List<XFile> files;
      switch (mediaType) {
        case AcceptedMediaType.image:
        ...
        case AcceptedMediaType.video:
          final file = await _picker.pickVideo(source: source);
          files = file == null ? const [] : [file];
        case AcceptedMediaType.mixed:
          if (allowMultiple) {
        ...
        case AcceptedMediaType.unsupported:
          if (mounted) {
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('当前仅支持选择图片或视频')));
          }
          return const [];
      }
      return files
          .map((file) => Uri.file(File(file.path).absolute.path).toString())
          .toList();
    } on PlatformException catch (error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('无法选择文件：${error.message ?? error.code}')),
        );
      }
      return const [];
    }
