...
function CircleDetail() {
  ...
  const circlePosts = useMemo(() => {
    if (!circle) return [];
    const list = posts.filter((p) => p.circle === circle.name);
    // ensure some content even when circle has no matching posts
            ...
            {tab === "recommend" && (
              <div className="mb-5 space-y-2">
                {pinned.map((p: { title: string; meta: string }) => (
                  ...
                  >
                    <Pin className="h-4 w-4 shrink-0 text-text-tertiary" strokeWidth={1.75} />
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-[14px] font-medium text-foreground">
                        {p.title}
                      </div>
                      <div className="text-[11.5px] text-text-tertiary">{p.meta}</div>
                    </div>
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-[92px] space-y-4">
              <section className="rounded-[16px] border border-[color:var(--border)] bg-white/70 p-5">
                <h3 className="mb-2 text-[13px] font-semibold text-foreground">关于圈子</h3>
              ...
              <section className="rounded-[16px] border border-[color:var(--border)] bg-white/70 p-5">
                <h3 className="mb-3 text-[13px] font-semibold text-foreground">圈子公约</h3>
              ...
              <section className="rounded-[16px] border border-[color:var(--border)] bg-white/70 p-5">
                <h3 className="mb-3 text-[13px] font-semibold text-foreground">活跃成员</h3>
                <ul className="space-y-2.5">
                  {["林深", "周航", "Kai", "老周的桌面", "夜航船"].map((n) => (
                    <li key={n} className="flex items-center gap-2.5">
                      ...
                      <span className="flex-1 text-[13px] text-foreground">{n}</span>
                      <span className="text-[11px] text-text-tertiary">活跃</span>
                    </li>
                  ))}
                </ul>
              </section>
