...
export type SearchSuggestionType = "hot" | "keyword" | "circle" | "post";
...
export function getSearchSuggestions(q = ""): Promise<SearchSuggestionsResponse> {
    const query = q.trim();
    if (useMockApi) return getMockSearchSuggestions(query);
    const params = new URLSearchParams({ q: query });
    return authRequest<SearchSuggestionsResponse>(`/api/v1/search/suggest?${params.toString()}`);
}
