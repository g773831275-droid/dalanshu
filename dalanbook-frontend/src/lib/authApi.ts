...
function asApiPayload(payload: unknown): ApiPayload {
...
function unwrap<T>(payload: unknown, response: Response): T {
    const apiPayload = asApiPayload(payload);
    if (!response.ok) {
        throw new AuthApiError(apiPayload.message || apiPayload.msg || "请求失败", response.status);
    }
    if (typeof apiPayload.code === "number") {
        if (apiPayload.code !== 200) {
            throw new AuthApiError(
                apiPayload.msg || apiPayload.message || "请求失败",
                apiPayload.code,
            );
        }
        return apiPayload.data as T;
    }
    if (apiPayload.code && apiPayload.message) {
        throw new AuthApiError(apiPayload.message, apiPayload.code);
    }
    return payload as T;
}
...
export async function getCaptcha(): Promise<Captcha> {
    if (useMockApi) return getMockCaptcha();
    const captcha = await authRequest<Captcha>("/api/v1/auth/code");
    return {
        ...captcha,
        img:
            captcha.img && !captcha.img.startsWith("data:")
...
export async function getMe(): Promise<AuthUser> {
    if (useMockApi) return getMockMe();
...
function browserInfo(ua: string) {
    ...
    if (/Firefox\//.test(ua))
        return { browser: "Firefox", version: matchVersion(ua, /Firefox\/([\d.]+)/) };
    if (/Chrome\//.test(ua))
        return { browser: "Chrome", version: matchVersion(ua, /Chrome\/([\d.]+)/) };
    if (/Safari\//.test(ua))
        return { browser: "Safari", version: matchVersion(ua, /Version\/([\d.]+)/) };
    return { browser: "Unknown", version: "" };
}

function osInfo(ua: string, platform?: string) {
    if (/Android/.test(ua))
        return { os: "Android", version: matchVersion(ua, /Android\s([\d.]+)/) };
    if (/iPhone|iPad|iPod/.test(ua))
        return { os: "iOS", version: matchVersion(ua, /OS\s([\d_]+)/) };
    if (/Windows/.test(ua) || platform === "Windows")
        return { os: "Windows", version: matchVersion(ua, /Windows NT\s([\d.]+)/) };
    if (/Mac OS X/.test(ua) || platform === "macOS")
        return { os: "macOS", version: matchVersion(ua, /Mac OS X\s([\d_]+)/) };
...
export async function reportWebDevice(): Promise<DeviceSummary | undefined> {
    ...
    const device = deviceInfo(
        ua,
        uaData?.mobile,
        typeof highEntropy.model === "string" ? highEntropy.model : "",
    );
